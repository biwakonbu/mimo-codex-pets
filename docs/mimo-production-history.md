...
- `CodexSessionSummarizer` derives a small safe `workSummary` from sanitized
  ...
  Codex is working on, such as `動作中で、作業内容の説明をテスト中` or
  `停止中で、作業内容の説明をレビューできます`, without exposing raw commands,
...
- `CodexMimoDialoguePrompt` can use an ephemeral Codex app-server session to
  rewrite the already-sanitized session title, state, activity kind, and
  `workSummary` into a warmer one-sentence Mimo bubble. The generated
  `mimoSpeech` is safety-checked again, cached, and throttled per session, while
...
- production speech bubbles now use nearly the full 432pt panel width with a
  ...
  secondary session rows to two lines, and `PetSpeechBubblePaginator` advances
  overlong Mimo speech as timed pages before the next session update is shown
